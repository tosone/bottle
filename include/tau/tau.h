...
static TAU_UInt64 tauStatsNumWarnings;
...
*/
static int checkIsInsideTestSuite;
static int hasCurrentTestFailed;
#ifndef TAU_NO_TESTING
static int shouldFailTest;
static int shouldAbortTest;
...
static struct tauTestStateStruct tauTestContext;
...
#define TAU_TEST_INITIALIZER(f)                                                                                                               \
  ...
  __pragma(comment(linker, "/include:" TAU_SYMBOL_PREFIX #f "_")) TAU_C_FUNC __declspec(allocate(".CRT$XCU")) void(__cdecl * f##_)(void) = f; \
  static void __cdecl f(void)
#else
#define TAU_TEST_INITIALIZER(f)                     \
...
static inline int TAU_ATTRIBUTE_(format(printf, 2, 3)) tauColouredPrintf(int colour, const char *fmt, ...);
static inline int TAU_ATTRIBUTE_(format(printf, 2, 3)) tauColouredPrintf(int colour, const char *fmt, ...) {
  va_list args;
  char buffer[256];
  int n;
...
static inline int tauShouldDecomposeMacro(char const *actual, char const *expected, int isStringCmp) {
  ...
  // '(', ')', '-'
    ...
    if (strchr(actual, '(') != NULL || strchr(expected, '(') != NULL || actual[0] != '"' || expected[0] != '"') {
      return 1;
    }
  }
  return 0;
...
#define TAU_OVERLOAD_PRINTER(val) tauPrintf(_Generic((val), char : "'%c'", char * : "%s", unsigned char : "%hhu", short : "%hd", unsigned short : "%hu", int : "%d", unsigned int : "%u", long : "%ld", long long : "%lld", unsigned long : "%lu", unsigned long long : "%" TAU_PRIu64, float : "%f", double : "%f", long double : "%Lf", void * : "%p"), (val))
...
#define __TAUCMP_STRN__(actual, expected, n, cond, ifCondFailsThenPrint, actualPrint, macroName, failOrAbort)                        \
  do {                                                                                                                               \
    ...
    if (strncmp(actual, expected, n) cond 0) {                                                                                       \
      ...
      tauPrintf("  Expected : \"%.*s\" %s \"%.*s\"\n", TAU_CAST(int, n), actual, #ifCondFailsThenPrint, TAU_CAST(int, n), expected); \
      tauPrintf("    Actual : %s\n", #actualPrint);                                                                                  \
      failOrAbort;                                                                                                                   \
      return;                                                                                                                        \
    }                                                                                                                                \
...
#define STATIC_REQUIRE_FALSE(...) WARN(Usage of "STATIC_REQUIRE_FALSE" is not supported in C source files.)
#endif // __cplusplus
...
#define TEST(TESTSUITE, TESTNAME)                                                                                                                                                              \
  ...
  TAU_TEST_INITIALIZER(tau_register_##TESTSUITE##_##TESTNAME) {                                                                                                                                \
    ...
    tauTestContext.tests = TAU_PTRCAST(struct tauTestSuiteStruct *, tau_realloc(TAU_PTRCAST(void *, tauTestContext.tests), sizeof(struct tauTestSuiteStruct) * tauTestContext.numTestSuites)); \
...
#define TEST_F_SETUP(FIXTURE) static void __TAU_TEST_FIXTURE_SETUP_##FIXTURE(struct FIXTURE *tau)

#define TEST_F_TEARDOWN(FIXTURE) static void __TAU_TEST_FIXTURE_TEARDOWN_##FIXTURE(struct FIXTURE *tau)
...
static int tauShouldFilterTest(const char *filter, const char *testcase) {
  if (filter) {
    ...
    if ((*filter_curr != TAU_NULLCHAR) || ((*testcase_curr == TAU_NULLCHAR) && ((filter == filter_curr) || (filter_curr[-1] != '*')))) {
      // We have a mismatch
      return 1;
    }
  }
...
static void tauRunTests() {
  ...
  for (TAU_Ull i = 0; i < tauTestContext.numTestSuites; i++) {
    ...
    if (hasCurrentTestFailed == 1) {
      ...
      tauStatsFailedTestSuites = TAU_PTRCAST(TAU_Ull *, tau_realloc(TAU_PTRCAST(void *, tauStatsFailedTestSuites), sizeof(TAU_Ull) * tauStatsNumFailedTestSuites));
...
#define TAU_ONLY_GLOBALS()               \
  ...
  static int shouldFailTest = 0;         \
  static int shouldAbortTest = 0;        \
  static TAU_UInt64 tauStatsNumWarnings = 0;

// If a user wants to define their own `main()` function, this _must_ be at the very end of the functtion
#define TAU_NO_MAIN()                                          \
  static struct tauTestStateStruct tauTestContext = {0, 0, 0}; \
  TAU_ONLY_GLOBALS()
...
#define TAU_MAIN()                                                           \
  ...
  TAU_ONLY_GLOBALS()                                                         \
                                                                             \
  int main(int argc, char **argv) { return tau_main(argc, argv); }
